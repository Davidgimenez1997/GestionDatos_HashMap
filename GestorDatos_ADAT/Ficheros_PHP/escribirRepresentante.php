...
require 'jsonEsperadoRepresentate.php';
...
if(isset($parameters)){
	...
	if(JSONCorrectoAnnadirRepresentante($mensajeRecibido)){

		$repesentante = $mensajeRecibido["representanteAnnadir"];

		$id = $repesentante["id"];
		$nombre = $repesentante["nombre"];
		$edad = $repesentante["edad"];


		$query  = "INSERT INTO  representantes (Id,Nombre,Edad) ";
		$query .= "VALUES ('$id','$nombre','$edad')";

		$result = $conn->query ( $query );

		if (isset ( $result ) && $result) { // Si pasa por este if, la query está está bien y se ha insertado correctamente
			$arrMensaje["estado"] = "ok";
			$arrMensaje["mensaje"] = "Representante insertado correctamente";
