...
require 'jsonEsperadoRepresentateModificar.php';
...
if(isset($parameters)){
	...
	if(JSONCorrectoModificarRepresentante($mensajeRecibido)){
		...
		$repesentante = $mensajeRecibido["representanteModificar"];
		...
		$query  = "UPDATE representantes SET Nombre = '$nombre' ,Edad = '$edad' WHERE Id= '$id' ";

		$result = $conn->query ( $query );

		if (isset ( $result ) && $result) { // Si pasa por este if, la query está está bien y se ha insertado correctamente
			$arrMensaje["estado"] = "ok";
			$arrMensaje["mensaje"] = "Representante actualizado correctamente";
